...
import { ArrowLeft, Users, FileText, Gavel, Info, Plus, Loader2, X, Trash2, Globe2, Scale, Map, KeyRound, Copy, CheckCircle2, ClipboardList, Sparkles, AlertTriangle, Link2, FolderOpen } from "lucide-react"
...
function Resumen({ bien }: { bien: Bien }) {
  return (
    <div className="space-y-5">
      ...
      <VincularDocumentos bien={bien} />
    </div>
  )
}

// ─── Vincular expediente GD y carpeta física ──────────────────────────────────

type ExpedienteItem = { id: string; codigo: string; nombre: string }
type CarpetaItem    = { id: string; codigo: string; titulo: string }

function VincularDocumentos({ bien }: { bien: Bien }) {
  const router = useRouter()
  const [open, setOpen]           = useState(false)
  const [expedientes, setExps]    = useState<ExpedienteItem[]>([])
  const [carpetas, setCarpetas]   = useState<CarpetaItem[]>([])
  const [expId, setExpId]         = useState(bien.expediente?.id ?? "")
  const [carpetaId, setCarpetaId] = useState(bien.carpetaFisica?.id ?? "")
  const [loadingOpts, setLoadOpts]= useState(false)
  ...
  async function abrirPanel() {
    ...
    setLoadOpts(true)
    const [rExp, rCarp] = await Promise.all([
      fetch("/api/admin/gd/expedientes?limit=50").then(r => r.json()).catch(() => ({ expedientes: [] })),
      fetch("/api/admin/gd/archivo/carpetas?limit=50").then(r => r.json()).catch(() => ({ carpetas: [] })),
    ])
    setExps(rExp.expedientes ?? [])
    setCarpetas(rCarp.carpetas ?? [])
    setLoadOpts(false)
  }
  ...
  function guardar() {
    setError(null)
    start(async () => {
      const r = await fetch(`/api/admin/frisco/bienes/${bien.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          expedienteId:    expId    || null,
          carpetaFisicaId: carpetaId || null,
        }),
      })
    ...
    <div className="border rounded-lg p-4 bg-slate-50 text-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-slate-700 flex items-center gap-2">
          <Link2 className="w-4 h-4 text-indigo-600" /> Vínculos documentales
        </h3>
        <button onClick={abrirPanel}
          className="px-3 py-1.5 text-xs rounded-md border border-indigo-300 text-indigo-700 hover:bg-indigo-50">
          {open ? "Cancelar" : "Editar vínculos"}
        </button>
      </div>
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <dt className="text-xs font-medium text-slate-500 uppercase flex items-center gap-1">
              <FolderOpen className="w-3.5 h-3.5" /> Expediente GD
            </dt>
            <dd className="text-slate-800 mt-0.5">
              {bien.expediente ? `${bien.expediente.codigo} — ${bien.expediente.nombre}` : <span className="text-slate-400">Sin vincular</span>}
            </dd>
          </div>
          <div>
            <dt className="text-xs font-medium text-slate-500 uppercase flex items-center gap-1">
              <FolderOpen className="w-3.5 h-3.5" /> Carpeta física GA
            </dt>
            <dd className="text-slate-800 mt-0.5">
              {bien.carpetaFisica ? `${bien.carpetaFisica.codigo} — ${bien.carpetaFisica.titulo}` : <span className="text-slate-400">Sin vincular</span>}
            </dd>
          </div>
        </dl>
      )}
      ...
      {open && (
        <div className="space-y-3">
          {loadingOpts ? (
            <div className="flex items-center gap-2 text-slate-500"><Loader2 className="w-4 h-4 animate-spin" /> Cargando opciones…</div>
          ) : (
            <>
              <label className="block">
                <span className="text-xs font-medium text-slate-600 uppercase">Expediente GD</span>
                <select value={expId} onChange={e => setExpId(e.target.value)}
                  className="mt-1 w-full border rounded-md px-2 py-1.5 text-sm">
                  <option value="">— Sin vincular —</option>
                  {expedientes.map(e => <option key={e.id} value={e.id}>{e.codigo} — {e.nombre}</option>)}
                </select>
                {expedientes.length === 0 && <p className="text-xs text-amber-600 mt-1">No hay expedientes disponibles. Cree uno en Gestión Documental.</p>}
              </label>
              <label className="block">
                <span className="text-xs font-medium text-slate-600 uppercase">Carpeta física GA</span>
                <select value={carpetaId} onChange={e => setCarpetaId(e.target.value)}
                  className="mt-1 w-full border rounded-md px-2 py-1.5 text-sm">
                  <option value="">— Sin vincular —</option>
                  {carpetas.map(c => <option key={c.id} value={c.id}>{c.codigo} — {c.titulo}</option>)}
                </select>
                {carpetas.length === 0 && <p className="text-xs text-amber-600 mt-1">No hay carpetas físicas. Cree una en Archivo Físico.</p>}
              </label>
            </>
          )}
          {error && <p className="text-sm text-red-600">{error}</p>}
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={() => setOpen(false)} className="px-3 py-1.5 text-xs border rounded-md">Cancelar</button>
            <button type="button" onClick={guardar} disabled={pending || loadingOpts}
              className="px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-md disabled:opacity-50 flex items-center gap-1">
              {pending && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Guardar vínculos
            </button>
          </div>
        </div>
      )}
...
type AnalisisDiscrepancia = {
  discrepancias: Array<{
    campo: string; fuenteA: string; valorA: string
    fuenteB: string; valorB: string; descripcion: string
    severidad: "INFO" | "ALERTA" | "CRITICA"
  }>
  resumenFiscal: string | null
...
function InteropTab({ bien }: { bien: Bien }) {
  ...
  const [analisis,   setAnalisis]   = useState<AnalisisDiscrepancia | null>(null)
  ...
  async function consultar(
    ...
    // Limpiar análisis previo al re-consultar
    setAnalisis(null)
  ...
  async function analizarConIA() {
    ...
    setAnalisisError(null)
    try {
      const res = await fetch("/api/admin/frisco/interop/analizar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bienInterno: {
            codigo:         bien.codigo,
            tipo:           bien.tipo,
            estadoJuridico: bien.estadoJuridico,
            estadoFisico:   bien.estadoFisico,
            ubicacion:      bien.ubicacion,
            avaluoVigente:  bien.avaluoVigente,
          },
          snr:      snr.data      ?? null,
          igac:     igac.data     ?? null,
          fiscalia: fiscalia.data ?? null,
        }),
      })
      const j = await res.json()
      if (!res.ok) { setAnalisisError(j.error || "Error al analizar"); return }
      setAnalisis(j.analisis)
    } catch {
      setAnalisisError("Error de red al analizar")
    } finally {
      setAnalizando(false)
    }
  }

  const puedeAnalizar = (snr.data != null || igac.data != null || fiscalia.data != null)
    && !snr.pending && !igac.pending && !fiscalia.pending
      ...
      <div className="border border-violet-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 bg-violet-50 border-b border-violet-200">
          <h3 className="text-sm font-semibold text-violet-800 flex items-center gap-2">
            ...
            Análisis de discrepancias con IA
          </h3>
          <button
            onClick={analizarConIA}
            disabled={!puedeAnalizar || analizando}
            title={!puedeAnalizar ? "Consulta al menos un servicio antes de analizar" : undefined}
            className="px-3 py-1.5 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
          >
            {analizando
              ? <><Loader2 className="w-3 h-3 animate-spin" />Analizando…</>
              : <><Sparkles className="w-3 h-3" />Analizar discrepancias</>
            }
          </button>
        </div>

        <div className="p-4 text-sm space-y-4">
          {!puedeAnalizar && !analisis && (
            <p className="text-gray-400 italic">Consulta al menos una fuente externa para habilitar el análisis.</p>
          )}
          {puedeAnalizar && !analisis && !analizando && !analisisError && (
            <p className="text-gray-400 italic">Pulsa "Analizar discrepancias" para cruzar las fuentes consultadas.</p>
          )}
          {analisisError && (
            <div className="px-3 py-2 bg-red-50 border border-red-200 text-red-700 rounded">{analisisError}</div>
          )}

          {analisis && (
            <div className="space-y-4">
              {/* Nivel de riesgo + resumen */}
              <div className={`flex items-start gap-3 border rounded-xl px-4 py-3 ${NIVEL_COLOR[analisis.nivelRiesgo]}`}>
                <div className="text-xs font-bold uppercase tracking-wide mt-0.5 shrink-0">
                  Riesgo {analisis.nivelRiesgo}
                </div>
                <p className="text-sm">{analisis.resumenGeneral}</p>
              </div>

              {/* Resumen proceso fiscal */}
              {analisis.resumenFiscal && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Proceso fiscal</p>
                  <p className="text-sm text-slate-700">{analisis.resumenFiscal}</p>
                </div>
              )}
              ...
              {analisis.discrepancias.length > 0 ? (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                    {analisis.discrepancias.length} discrepancia(s) detectada(s)
                  </p>
                  {analisis.discrepancias.map((d, i) => (
                    <div key={i} className={`border rounded-xl px-4 py-3 ${SEV_COLOR[d.severidad]}`}>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-bold uppercase tracking-wide opacity-70">{d.severidad}</span>
                        <span className="font-semibold text-sm">{d.campo}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-x-4 text-xs mb-1">
                        <div><span className="font-medium">{d.fuenteA}:</span> {d.valorA}</div>
                        <div><span className="font-medium">{d.fuenteB}:</span> {d.valorB}</div>
                      </div>
                      <p className="text-xs opacity-80">{d.descripcion}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-emerald-700 flex items-center gap-2">
                  ...
                  No se detectaron discrepancias entre las fuentes consultadas.
                </p>
              )}

              <p className="text-[11px] text-gray-400">
                IA sugiere — el funcionario decide. Modelo: {analisis.modelo} ({analisis.proveedor})
              </p>
            </div>
          )}
        </div>
